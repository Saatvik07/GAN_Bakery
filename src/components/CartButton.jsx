...
function CartButton() {
	...
	return (
		<div className='fixed right-4 bottom-4 md:right-8 md:bottom-8 lg:right-12 lg:bottom-12'>
			<button className=' bg-accent-100 rounded-full p-3 md:p-4 lg:p-5 relative'>
				{cart.length > 0 ? (
					<div className='bg-textColor text-accent-100 text-sm font-bold font-body absolute p-1 px-2 top-0 right-0 rounded-full -translate-y-2'>
						{getShortenedCartLength(cart)}
					</div>
				) : null}
				<img src={Cart} alt='cart' className='w-5 h-5 md:w-6 md:h-6 lg:w-8 lg:h-8' />
				{/*
